...
from dataset.config import voc, coco
from ssd.utils_ssd.prior_box import PriorBox
from ssd.utils_ssd.l2norm import L2Norm
from ssd.utils_ssd.detection import Detect
from ssd.utils_ssd.ops import weights_init
...
def build_ssd(phase, size=300, num_classes=21):
    if phase != "test" and phase != "train":
        raise Exception("Error: phase not recognized!")

    if size != 300:
        raise Exception("Only SSD300 is supported!")
...
if __name__ == '__main__':
    ...
    img = torch.randn((4, 3, 300, 300))
    loc, conf, priors = net.forward(img)
